    ...
    def insert_data(self,data,list_header:List[str] = []):
        ...
        for account in data:
            ...
            for key in account:
                ...
                count_column+=1
            if account.get("state") not in self.list_state and len(str(account.get("state")))>0:
                self.list_state.append(account.get("state"))
        ...
        for row,account in enumerate(data):    
            for index,key in enumerate(list_header):
                newitem = QTableWidgetItem(str(account.get(key)))
